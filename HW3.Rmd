...
**Note that Kenny and Paul have ***NOT*** worked on any previous homework assignments.**
...
__The following code estimates the ith-degree polynomial, where i ranges from 1 to 10. Thus, we see that from the returned summary output we get estimates for the coefficients up to the 5th degree polynomial. (Note that had we successfully estimated every part of the model for all 10 coefficients, we would have simply expanded our for-loop to contain more iterations.) It appears that the 5th degree__
...
```{r problem 7}
for (i in 1:20){
lm.poly.1 = lm(median.temp.anom ~ poly(hadcrut$yearfrac,i,raw = FALSE), data = hadcrut) 
pvals[i] = (summary(lm.poly.1)$coefficients[i,4])
}
pvals
which(pvals > 0.05)
lm.poly.final = lm(median.temp.anom ~ poly(hadcrut$yearfrac,10,raw = FALSE), data = hadcrut)
...
_Based on the plots below, we can see that...
